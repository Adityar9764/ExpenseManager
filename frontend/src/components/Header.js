...
const Header = ({user, setUser}) => {
  ...
  // Fetch user info from backend directly
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await fetch('http://localhost:3883/api/auth/getUser', {
          method: 'GET',
          headers: {
  ...
  const handleShowLogout = () => {
    setUser(null);  // Log out user by resetting the state
    navigate("/login");
  }
        ...
        <Navbar className="navbarCSS" collapseOnSelect expand="lg" style={{ position: 'relative', zIndex: "2 !important" }}>
          ...
          <div>
            <Navbar.Collapse id="responsive-navbar-nav" style={{color: "white"}}>
              {user ? (
                <>
                  <Nav.Item className="ml-2 nav-btn">
                    Welcome, {user.name}!
                  </Nav.Item>
                  <Nav>
                    <Button variant="primary" onClick={handleShowLogout} className="ml-2 nav-btn">Logout</Button>
                  </Nav>
                </>
              ) : (
                <>
                  <Nav.Link as={Link} to="/register">
                    <Button variant="primary" className="ml-2 nav-btn">Register</Button>
                  </Nav.Link>

                  <Nav.Link as={Link} to="/login">
                    <Button variant="primary" className="ml-2 nav-btn">Login</Button>
                  </Nav.Link>
                </>
              )}
            </Navbar.Collapse>
